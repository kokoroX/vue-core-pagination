<template>
  <ul :class="`${prefixCls}`" unselectable="unselectable">
    <span class="ant-pagination-total-text">
    ...
    <li :title="`最后一页: ${pageCount}`" :class="[`${prefixCls}-item`, {[`${prefixCls}-item-active`]: current === pageCount}]" v-show="pageCount" @click="jumpTo(pageCount)"><a>{{pageCount}}</a></li>
